...
## This is a script to parse results from PhyloAcc. It uses parsed and copied files by 05_parse_PhyloAcc.sh
...
cnees_labeled <- cneeantbirds_complete %>% mutate(Accel_10_0 = bf1 > 10, Spec_10_0 = bf1 > 10 & bf2 > 0) %>% 
  mutate(Accel_20_0 = bf1 > 20, Spec_20_0 = bf1 > 20 & bf2 > 0)

table(cnees_labeled$Spec_10_0)["TRUE"]
table(cnees_labeled$Spec_20_0)["TRUE"]
```

# 9. Selecting Top Accelerating CNEEs

```{r, message=FALSE, warning = FALSE}
bf.1_10_0 <- cnees_labeled %>%
  filter(Spec_10_0 == T) %>%
  arrange(chr,start)
